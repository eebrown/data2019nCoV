...
The following plot is to test the hypothesis that Mother's Day gatherings on May 10 led to a spike in cases. Here we look at the ratio of F:M in non-outbreak cases in non-teenagers. Presumably mother's day gatherings involve more females if we assume it is celebrated equally by sons and daughters (or at least not moreso by sons), and that fathers are not always present. Shading on graph reflects 1-14 days after event, which reflects incubation period (this uses episode date, not reported date).
